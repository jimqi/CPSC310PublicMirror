/**
 * This is the class used to convert an XML text into a JSONObject
 * and to convert a JSONObject into an XML text.
 */
package server;
...
import server.XMLConverter;
import server.XMLTokener;
				...
				if (token instanceof String) {
					...
					if (token == EQ) {
						...
						jsonobject.accumulate(string,
								XMLConverter.stringToValue((String) token));
						token = null;
					} else {
						jsonobject.accumulate(string, "");
					}
						...
						if (token == null) {
							...
							if (string.length() > 0) {
								jsonobject.accumulate("content",
										XMLConverter.stringToValue(string));
							}
